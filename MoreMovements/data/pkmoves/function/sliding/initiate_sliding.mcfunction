...
execute if entity @s[tag=pkmoves.is_in_air] at @s anchored eyes unless block ^ ^ ^1.2 #pkmoves:not_grabable run return fail
...
execute if score y_facing pkmoves.global matches ..-55000 run return fail
execute if score y_facing pkmoves.global matches 55000.. run return fail
...
execute at @s if score vmt pkmoves.global matches -20..-8 run summon item_display ~ ~-1 ~ {Tags:["pkmoves.init","pkmoves.slide_hitbox"],Passengers:[{id:"minecraft:shulker",NoGravity:1b,Silent:1b,Invulnerable:1b,DeathLootTable:"asdasd",NoAI:1b,AttachFace:1b,Color:14b}]}
